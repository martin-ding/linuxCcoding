...
int main()
{
    ...
    for (i = 0; i < 20; ++i)
    {
        num[i] = i;
        printf("add task %d\n", i);
        threadpool_add_task(thp, process, (void *) num[i]);//向线程池中添加任务
    }
    sleep(10);//等子线程完成任务
...
threadpool_t *threadpool_create(int min_thr_num, int max_thr_num, int queue_max_size)
{
    ...
    do
    {
        ...
        pool->queue_front = 0;
        pool->queue_rear = 0;
        pool->queue_size = 0;//初始任务队列
        pool->queue_max_size = queue_max_size;
        pool->shutdown = false;

        /*根据最大线程数，给工作线程数组开空间，清0  线程数组 */
...
int threadpool_destroy(threadpool_t *pool)
{
    ...
    //如果还有线程再干活也会在干完活之后 因为 pool->shutdown 变成了true 被回收
    //threadpool_thread 代码写的很巧妙
...
void * admin_thread(void *threadpool)
{
    ...
    while (!pool->shutdown)
    {
        ...
        int queue_size = pool->queue_size;               /*任务数，现有的任务数量*/
        ...
        printf("admin busy live queue_size -%d--%d---%d \n", busy_thr_num, live_thr_num, queue_size);
        /*创建新线程 实际任务数量大于最小正在等待的任务数量，存活线程数小于最大线程数*/
        // 只要现有的任务数超过了一定的阈值 MIN_WAIT_TASK_NUM 就新增线程
        if (queue_size >= MIN_WAIT_TASK_NUM && live_thr_num <= pool->max_thr_num)
        {
            ...
            {
                if (pool->threads[i] == 0 || !is_thread_alive(pool->threads[i]))//表示曾经这个数组坑被占用过
                {
                    pthread_create(&(pool->threads[i]), NULL, threadpool_thread, (void *)pool);
                    add++;
                    pool->live_thr_num++;
                    printf("new thread ----------------------ox%x-\n", pool->threads[i]);
                }
            }
...
void * threadpool_thread(void *threadpool)
{
    ...
    while (true)
    {
        ...
        // 其实在其他语言中如果有try catch 应该在finally 里面进行解锁
    ...
    pthread_exit(NULL);//while 循环没有break的时候，应该没有可能执行到这个语句
}

/*向线程池的任务队列中添加一个任务*/
int threadpool_add_task(threadpool_t *pool, void *(*function)(void *arg), void *arg)
{
    pthread_mutex_lock(&(pool->lock));//整个pool 锁住
